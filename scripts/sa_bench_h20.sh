...
| Config | Scenario | EP | CONC | Total Tput | Output Tput | Interac. | TPOT (ms) | TTFT (ms) | DAR |
|--------|----------|-----|------|------------|-------------|----------|-----------|-----------|-----|
EOF
...
try:
    ...
    print(f'| {label} | {scenario} | {ep} | {conc} | {total_tps:.1f} | {out_tps:.1f} | {interactivity:.2f} | {tpot_p50:.1f} | {ttft_p50:.0f} | {dar_str} |')
except Exception as e:
