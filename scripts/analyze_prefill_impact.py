...
def save_xlsx(gaps, benchmark, output_path):
    ...
    # Compute breakdowns for normal vs interrupted
    sum_gap_x_bs_normal = sum(g["gap_ms"] * g["bs"] for g in normal)
    sum_gap_x_bs_interrupted = sum(g["gap_ms"] * g["bs"] for g in interrupted)
    sum_bs_normal = sum(g["bs"] for g in normal)
    sum_bs_interrupted = sum(g["bs"] for g in interrupted)
    avg_bs_normal = sum_bs_normal / len(normal) if normal else 0
    avg_bs_interrupted = sum_bs_interrupted / len(interrupted) if interrupted else 0
    sum_gap_ms_normal = sum(g["gap_ms"] for g in normal)
    sum_gap_ms_interrupted = sum(g["gap_ms"] for g in interrupted)

    summary_rows.append(row("", "", "", "", ""))
    summary_rows.append(row("--- BS-WEIGHTED AVERAGE CALCULATION (PRECISE) ---", "", "", "", ""))
    ...
    summary_rows.append(row("", "", "", "", "Formula: bs_weighted_avg = sum(gap_ms * bs) / sum(bs)"))
    summary_rows.append(row("", "", "", "", ""))
    summary_rows.append(row("", "", "", "", "Step 1: sum(gap_ms * bs) broken down by normal vs interrupted"))
    summary_rows.append(row("sum_gap_ms_normal", round(sum_gap_ms_normal, 2), "", "ms", f"normal_gaps({len(normal)}) x normal_gap_mean({round(sum_gap_ms_normal/len(normal),2)}ms) = {round(sum_gap_ms_normal,2)}ms total"))
    summary_rows.append(row("avg_bs_normal", round(avg_bs_normal, 2), "", "count", f"sum_bs_normal({sum_bs_normal}) / normal_gaps({len(normal)}) = {round(avg_bs_normal,2)} avg batch size for normal gaps"))
    summary_rows.append(row("sum_gap_x_bs_normal", round(sum_gap_x_bs_normal, 2), "", "ms*count", f"sum(gap_ms * bs) for {len(normal)} normal gaps = {round(sum_gap_x_bs_normal,2)}"))
    summary_rows.append(row("sum_bs_normal", sum_bs_normal, "", "count", f"sum(bs) for {len(normal)} normal gaps = {sum_bs_normal} tokens from normal decodes"))
    summary_rows.append(row("", "", "", "", ""))
    summary_rows.append(row("sum_gap_ms_interrupted", round(sum_gap_ms_interrupted, 2), "", "ms", f"interrupted_gaps({len(interrupted)}) x interrupted_gap_mean({round(sum_gap_ms_interrupted/len(interrupted),2)}ms) = {round(sum_gap_ms_interrupted,2)}ms total"))
    summary_rows.append(row("avg_bs_interrupted", round(avg_bs_interrupted, 2), "", "count", f"sum_bs_interrupted({sum_bs_interrupted}) / interrupted_gaps({len(interrupted)}) = {round(avg_bs_interrupted,2)} avg batch size for interrupted gaps"))
    summary_rows.append(row("sum_gap_x_bs_interrupted", round(sum_gap_x_bs_interrupted, 2), "", "ms*count", f"sum(gap_ms * bs) for {len(interrupted)} interrupted gaps = {round(sum_gap_x_bs_interrupted,2)}"))
    summary_rows.append(row("sum_bs_interrupted", sum_bs_interrupted, "", "count", f"sum(bs) for {len(interrupted)} interrupted gaps = {sum_bs_interrupted} tokens from interrupted decodes"))
    summary_rows.append(row("", "", "", "", ""))
    summary_rows.append(row("", "", "", "", "Step 2: Sum up normal + interrupted"))
    summary_rows.append(row("sum_gap_x_bs", round(bs_weighted_sum, 2), "", "ms*count", f"= sum_gap_x_bs_normal({round(sum_gap_x_bs_normal,2)}) + sum_gap_x_bs_interrupted({round(sum_gap_x_bs_interrupted,2)}) = {round(bs_weighted_sum,2)}"))
    summary_rows.append(row("sum_bs", bs_total, "", "count", f"= sum_bs_normal({sum_bs_normal}) + sum_bs_interrupted({sum_bs_interrupted}) = {bs_total}"))
    summary_rows.append(row("", "", "", "", ""))
    summary_rows.append(row("", "", "", "", "Step 3: Divide"))
    summary_rows.append(row("bs_weighted_avg_gap", round(bs_weighted_avg, 2), "", "ms", f"= sum_gap_x_bs({round(bs_weighted_sum,2)}) / sum_bs({bs_total}) = {round(bs_weighted_avg,2)}ms"))
    if benchmark:
        delta = round(bs_weighted_avg - benchmark["mean_tpot_ms"], 2)
        summary_rows.append(row("delta_vs_mean_tpot", delta, "", "ms", f"= bs_weighted_avg({round(bs_weighted_avg,2)}) - benchmark_mean_tpot({round(benchmark['mean_tpot_ms'],2)}) = {delta}ms. Close to 0 = trace fully explains the TPOT."))

    summary_rows.append(row("", "", "", "", ""))
    summary_rows.append(row("--- RECONSTRUCTION FROM NORMAL/INTERRUPTED SPLIT (APPROXIMATE) ---", "", "", "", ""))
    summary_rows.append(row("", "", "", "", "Simplified formula using medians: reconstructed = (1-p) x normal_median + p x interrupted_median"))
    summary_rows.append(row("", "", "", "", "Less precise than bs_weighted (ignores bs variation and uses medians instead of all values) but easier to reason about."))
    if normal_vals and interrupted_vals:
        ...
        summary_rows.append(row("normal_fraction", round(1 - p_int, 4), "", "ratio", f"= normal_gaps({len(normal)}) / total_gaps({len(gaps)}) = {1-p_int:.4f}"))
        summary_rows.append(row("interrupted_fraction", round(p_int, 4), "", "ratio", f"= interrupted_gaps({len(interrupted)}) / total_gaps({len(gaps)}) = {p_int:.4f}"))
        summary_rows.append(row("reconstructed_tpot", round(recon, 2), "", "ms", f"= ({1-p_int:.4f} x {n_med:.2f}) + ({p_int:.4f} x {i_med:.2f}) = {(1-p_int)*n_med:.2f} + {p_int*i_med:.2f} = {recon:.2f}ms"))
    if benchmark:
