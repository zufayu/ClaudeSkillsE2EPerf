...
| **syntax-first** | 前端白屏/空白问题，第一步永远是 `node --check` 或等效语法检查，不是猜渲染逻辑 | 语法错误会静默杀死整个 script block，肉眼很难发现 `];` vs `);` 这类错误。机器检查 1 秒钟能省掉 30 分钟无效排查 |
| **prove-then-push** | 改完代码必须通过可执行验证（node 模拟运行、浏览器打开确认），不能只肉眼看 diff 就 push + deploy | 肉眼 review 会被"代码看起来合理"骗过，只有实际执行才能暴露隐藏的语法错误和运行时异常。尤其是修 bug 的 commit，修完后必须证明 bug 已消失 |
| **evidence-first** | 有可查的证据源（日志、文件、API、数据库）时，必须先查证据再下结论，禁止跳过证据直接构建假说 | 假说听起来合理不等于正确。未查证据就以确定语气呈现推测，会误导决策、浪费排查时间。正确做法：先穷尽可用证据源，再基于事实推导结论，推测部分必须明确标注为推测 |
