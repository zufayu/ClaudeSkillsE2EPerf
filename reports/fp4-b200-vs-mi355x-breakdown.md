...
##### PASS 级对比与优化预估（v33 更新）

> 以 MLA 和 MoE 两大 PASS 重新对比。B200 考虑 walltime（含 PDL + 双流 overlap），MI355X 为串行 kernel_sum。

| PASS | B200 walltime μs | MI355X 当前 μs | GAP | MI355X 优化后预估 | 优化手段 |
|------|----------------:|---------------:|----:|------------------:|---------|
| **MLA decode** | 20.3 | 31.95 | 11.65 | **~20.3** | MLA attn + reduce 融合（省 ~10.6μs） |
| **MoE** (含 routing) | 190.4 | 223.96 | 33.56 | **~190.4** | FlyDSL 替换 CK MoE GEMM（省 ~30μs） |
| **其他** (comm+norm+proj) | ~78 | ~108 | ~30 | ~98 | comm 融合（方案 A：rmsnorm_quant） |
| **总计** | **~288.5** | **~363.8** | **~75** | **~309** | |

> **MLA 优化（省 ~10.6μs）**：将 `mla_a8w8_qh16` (26.1μs) 和 `mla_reduce_v1` (5.9μs) 融合为单 kernel ~20.3μs，对标 B200 的 `fmhaSm100f` 单 kernel。需验证 CDNA4 register/LDS 容量是否支持 attention + head reduce 融合。
>
> **MoE 优化（省 ~30μs）**：当前 CK `kernel_moe_mxgemm_2lds` gate_up 126μs + down 63μs + quant_sort 4.8μs = 194μs。FlyDSL 方案预期将 MoE GEMM 效率提升至 B200 bmm_E2m1 水平（101.7+55.8=157.5μs），加 shared expert（23.4μs，当前融入 grouped GEMM）和 routing/quant 开销 ~10μs，预估 ~190μs。
>
> **优化后 vs B200**：MI355X ~309μs vs B200 ~288.5μs，差距从 75μs 缩小到 **~20μs**。剩余差距主要来自 PDL boundary savings (~12μs) 和 comm 通信效率差异。

> **小结：** 当前 MI355X 363.8μs vs B200 288.5μs 差距 75μs。MLA 融合（10.6μs）+ MoE FlyDSL（30μs）+ comm 融合（~10μs）共计约 50μs 优化空间，可将差距缩小到 ~20μs。剩余 20μs 中 ~12μs 是 PDL boundary 结构性差距（无硬件支持），~8μs 来自 allreduce 通信效率差异。
